...
% if max(MountAngleVar) > max(BetaVar)
%     MountAngleVar = linspace(-14,max(BetaVar),100);
% end
% 
% if min(MountAngleVar) < min(BetaVar)
%     MountAngleVar = linspace(min(BetaVar),14,100);
% end
...
figure
plot(MountAngleVar,MaxOutZPosObj,'LineWidth',2');
grid ('on')
grid ('minor')
title('MountAngle vs Max ZPos to percieve Obj')
xlabel('Max sensor height (Z-position (m))')
ylabel('MountAngle (Deg)')